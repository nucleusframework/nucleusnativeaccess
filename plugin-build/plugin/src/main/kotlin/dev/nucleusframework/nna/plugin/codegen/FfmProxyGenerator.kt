...
class FfmProxyGenerator {
    ...
    private fun generateEnumProxy(enum: KneEnum, module: KneModule, pkg: String): String = buildString {
        ...
        val constructor = buildString {
            enum.constructorParams.forEachIndexed { idx, param ->
                append("val ${param.name}: ${param.type.jvmTypeName}")
                if (idx + 1 != enum.constructorParams.size) append(", ")
            }
        }
        val constructorLine = if (constructor.isNotEmpty()) "($constructor)" else " "
        appendLine("enum class ${enum.simpleName}$constructorLine {")
